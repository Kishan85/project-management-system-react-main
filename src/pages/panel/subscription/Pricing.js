...
import { DOLLAR_SYMBOL, RUPEES_SYMBOL } from "../../../constants/myConstants";
const helpSupportIcon = svgData.filter((icon) => icon.slug === "help-support")[0];
const Pricing = ({ head, pricingTableData }) => {
  return (
    <React.Fragment>
      {!head && <Head title="Pricing"></Head>}
      <Content>
        <BlockHead size="lg">
                ...
                <Col md="3" key={item.id}>
                  <Card className={`card-bordered pricing text-center ${item.tags ? "recommend" : ""}`}>
                    ...
                    <div className="pricing-body">
                      ...
                      <div className="pricing-amount">
                        <div className="amount">
                          {DOLLAR_SYMBOL}{item.amount} <span>{item?.amount=="0.00"?"for a week":"/yr"}</span>
                        </div>
                        <span className="bill">{item.userNumber} User, Billed</span>
                      </div>
                      <ul className="pricing-features">
                        {item?.features?.map((feature, index) => (
                          <li key={index}>
                            {feature.feature}{" "}
                            {feature.available ? (
                              <Icon className={feature.available ? "available" : "unavailable"} name="check-thick" />
                            ) : (
                              <Icon className={feature.available ? "available" : "unavailable"} name="cross" />
                            )}
                          </li>
                        ))}
                      </ul>
                      <div className="pricing-action">
                        <Link to={"/auth-register"} state={item}>
                          {" "}
                          <Button color="primary">{item?.amount=="0.00"?"Request Demo" :"Select Plan"}</Button>
                        </Link>
                      </div>
                    </div>
                  </Card>
              ...
              <tbody className="tb-ftr-body">
                <tr className="tb-ftr-item">
                  ...
                  <td className="tb-ftr-plan">
                    <Icon name="check-thick" /> <span className="plan-name">Starter</span>
                  </td>
                  <td className="tb-ftr-plan">
                    <Icon name="check-thick" /> <span className="plan-name">Pro</span>
                  </td>
                  <td className="tb-ftr-plan">
                    <Icon name="check-thick" /> <span className="plan-name">Enterprise</span>
                  </td>
                </tr>
                <tr className="tb-ftr-item">
                ...
                <tr className="tb-ftr-item">
                  ...
                  <td className="tb-ftr-plan">
                    <Icon name="cross" /> <span className="plan-name">Starter</span>
                  </td>
                  <td className="tb-ftr-plan">
                    <Icon name="check-thick" /> <span className="plan-name">Pro</span>
                  </td>
                ...
                <tr className="tb-ftr-item">
                  ...
                  <td className="tb-ftr-plan">
                    <Icon name="cross" /> <span className="plan-name">Pro</span>
                  </td>
                  <td className="tb-ftr-plan">
                    <Icon name="check-thick" /> <span className="plan-name">Enterprise</span>
                  </td>
